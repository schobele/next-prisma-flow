...
export function usePostFormAutosave<T extends FlowPostCreate | FlowPostUpdate>(
  form: UseFormReturn<T>,
  mutation: UseMutationResult<FlowPost, Error, any>,
  options?: {
    enabled?: boolean;
    debounceMs?: number;
    fields?: string[];
  ...
  const saveField = useCallback(
    async (fieldName: string, value: any) => {
      console.log("[Post Autosave] saveField called:", {
        fieldName,
        value,
        enabled,
      ...
      if (lastSavedValues.current[fieldName] === value) {
        console.log("[Post Autosave] Value unchanged, skipping save");
        return;
      }
      ...
      console.log(
        "[Post Autosave] Setting debounce timer for",
        fieldName,
        "with delay",
        debounceMs,
      );
      debounceTimers.current[fieldName] = setTimeout(async () => {
        console.log("[Post Autosave] Debounce timer fired for", fieldName);
        ...
        if (lastSavedValues.current[fieldName] === currentValue) {
          console.log(
            "[Post Autosave] Value unchanged after debounce, skipping",
          );
          return;
        }
        ...
        if (!isValid) {
          console.log("[Post Autosave] Field validation failed");
        ...
        try {
          console.log("[Post Autosave] Calling mutation with:", {
          ...
          console.log("[Post Autosave] Mutation successful");
          ...
          console.error("[Post Autosave] Mutation failed:", error);
          setFieldSaveStates((prev) => ({ ...prev, [fieldName]: "error" }));
  ...
  useEffect(() => {
    console.log(
      "[Post Autosave] Setting up watch subscription, enabled:",
      enabled,
    );
    if (!enabled) return;

    const subscription = form.watch((value, { name, type }) => {
      console.log("[Post Autosave] Form change detected:", {
        name,
        type,
        isSystemUpdate: isSystemUpdate.current,
      });
      ...
      if (name && type === "change" && !isSystemUpdate.current) {
        ...
        console.log(
          "[Post Autosave] Triggering saveField for:",
          name,
          "with value:",
          fieldValue,
        );
    ...
    return () => {
      console.log("[Post Autosave] Cleaning up watch subscription");
...
export function usePostFormFieldTracking<
  T extends FlowPostCreate | FlowPostUpdate,
>(form: UseFormReturn<T>, original?: FlowPost) {
  const getFieldState = useCallback(
    (name: Path<T>) => {
      const fieldState = form.getFieldState(name);
      const currentValue = form.getValues(name);
      const originalValue = original?.[name as keyof FlowPost];
  ...
  const getDirtyFields = useCallback(() => {
    ...
    Object.keys(form.formState.dirtyFields).forEach((key) => {
      if ((form.formState.dirtyFields as any)[key]) {
        dirtyFields.push({
          field: key,
          original: original?.[key as keyof FlowPost],
...
export function usePostFormHistory<T extends FlowPostCreate | FlowPostUpdate>(
  form: UseFormReturn<T>,
  options?: {
...
export function usePostFieldDirect<T extends FlowPostCreate | FlowPostUpdate>(
  name: Path<T>,
  form: UseFormReturn<T>,
) {
