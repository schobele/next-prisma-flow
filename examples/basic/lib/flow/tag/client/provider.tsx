...
/**
 * Props for the Tag form component
 */
export interface FlowTagFormProps {
  children: ReactNode;
   ...
   */
  defaultValues?: Partial<FlowTagCreate | FlowTagUpdate>;

  /**
   * Callback when form is successfully submitted
   */
  onSuccess?: (data: FlowTag) => void | Promise<void>;
...
/**
 * Type-safe form component for Tag
 *
 * @example
 * ```tsx
 * <FlowTagForm mode="create">
 *   <TagFormField
 *     name="title"
 *     render={({ field }) => (
 *       <FormItem>
...
export function FlowTagForm({
  ...
  // Validate props
  if (mode === "update" && !id) {
    throw new Error(`FlowTagForm: id is required when mode is "update"`);
  }

  // Fetch existing data for update mode
  const { data: existingData, isLoading: isLoadingData } = useTag(id || "", {
  ...
  const form = useForm<FlowTagCreate | FlowTagUpdate>({
    resolver: zodResolver(
      mode === "create" ? TagCreateSchema : TagUpdateSchema,
    ),
  ...
  const handleSubmit = useCallback(
    async (data: FlowTagCreate | FlowTagUpdate) => {
      try {
        if (mode === "create") {
          await createMutation.mutateAsync(data as FlowTagCreate);
        } else if (mode === "update") {
          await updateMutation.mutateAsync(data as FlowTagUpdate);
        }
      } catch (error) {
  ...
  const handleAutosave = useCallback(
    async (data: Partial<FlowTagUpdate>) => {
      if (mode === "update" && id) {
        try {
          await updateMutation.mutateAsync(data as FlowTagUpdate);
        } catch (error) {
 ...
 */
export function FlowTagFormSubmit({
  children = "Submit",
  className,
  ...props
 ...
 */
export function FlowTagFormReset({
  children = "Reset",
  className,
  ...props
}: React.ButtonHTMLAttributes<HTMLButtonElement>) {
 ...
 */
export function FlowTagFormState({ className }: { className?: string }) {
