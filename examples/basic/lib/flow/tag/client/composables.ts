...
export function useTagFormAutosave<T extends FlowTagCreate | FlowTagUpdate>(
  form: UseFormReturn<T>,
  mutation: UseMutationResult<FlowTag, Error, any>,
  options?: {
    enabled?: boolean;
    debounceMs?: number;
    fields?: string[];
  ...
  const saveField = useCallback(
    async (fieldName: string, value: any) => {
      console.log("[Tag Autosave] saveField called:", {
        fieldName,
        value,
        enabled,
      ...
      if (lastSavedValues.current[fieldName] === value) {
        console.log("[Tag Autosave] Value unchanged, skipping save");
        return;
      }
      ...
      console.log(
        "[Tag Autosave] Setting debounce timer for",
        fieldName,
        "with delay",
        debounceMs,
      );
      debounceTimers.current[fieldName] = setTimeout(async () => {
        console.log("[Tag Autosave] Debounce timer fired for", fieldName);
        ...
        if (lastSavedValues.current[fieldName] === currentValue) {
          console.log(
            "[Tag Autosave] Value unchanged after debounce, skipping",
          );
          return;
        }
        ...
        if (!isValid) {
          console.log("[Tag Autosave] Field validation failed");
        ...
        try {
          console.log("[Tag Autosave] Calling mutation with:", {
          ...
          console.log("[Tag Autosave] Mutation successful");
          ...
          console.error("[Tag Autosave] Mutation failed:", error);
          setFieldSaveStates((prev) => ({ ...prev, [fieldName]: "error" }));
  ...
  useEffect(() => {
    console.log(
      "[Tag Autosave] Setting up watch subscription, enabled:",
      enabled,
    );
    if (!enabled) return;

    const subscription = form.watch((value, { name, type }) => {
      console.log("[Tag Autosave] Form change detected:", {
        name,
        type,
        isSystemUpdate: isSystemUpdate.current,
      });
      ...
      if (name && type === "change" && !isSystemUpdate.current) {
        ...
        console.log(
          "[Tag Autosave] Triggering saveField for:",
          name,
          "with value:",
          fieldValue,
        );
    ...
    return () => {
      console.log("[Tag Autosave] Cleaning up watch subscription");
...
export function useTagFormFieldTracking<
  T extends FlowTagCreate | FlowTagUpdate,
>(form: UseFormReturn<T>, original?: FlowTag) {
  const getFieldState = useCallback(
    (name: Path<T>) => {
      const fieldState = form.getFieldState(name);
      const currentValue = form.getValues(name);
      const originalValue = original?.[name as keyof FlowTag];
  ...
  const getDirtyFields = useCallback(() => {
    ...
    Object.keys(form.formState.dirtyFields).forEach((key) => {
      if ((form.formState.dirtyFields as any)[key]) {
        dirtyFields.push({
          field: key,
          original: original?.[key as keyof FlowTag],
...
export function useTagFormHistory<T extends FlowTagCreate | FlowTagUpdate>(
  form: UseFormReturn<T>,
  options?: {
...
export function useTagFieldDirect<T extends FlowTagCreate | FlowTagUpdate>(
  name: Path<T>,
  form: UseFormReturn<T>,
) {
