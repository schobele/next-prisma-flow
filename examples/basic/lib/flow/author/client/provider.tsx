...
/**
 * Props for the Author form component
 */
export interface FlowAuthorFormProps {
  children: ReactNode;
   ...
   */
  defaultValues?: Partial<FlowAuthorCreate | FlowAuthorUpdate>;

  /**
   * Callback when form is successfully submitted
   */
  onSuccess?: (data: FlowAuthor) => void | Promise<void>;
...
/**
 * Type-safe form component for Author
 *
 * @example
 * ```tsx
 * <FlowAuthorForm mode="create">
 *   <AuthorFormField
 *     name="title"
 *     render={({ field }) => (
 *       <FormItem>
 ...
 * ```
 */
export function FlowAuthorForm({
  children,
  mode,
  id,
  defaultValues,
  ...
  // Validate props
  if (mode === "update" && !id) {
    throw new Error(`FlowAuthorForm: id is required when mode is "update"`);
  }

  // Fetch existing data for update mode
  const { data: existingData, isLoading: isLoadingData } = useAuthor(id || "", {
  ...
  const form = useForm<FlowAuthorCreate | FlowAuthorUpdate>({
    resolver: zodResolver(
      mode === "create" ? AuthorCreateSchema : AuthorUpdateSchema,
    ),
  ...
  const handleSubmit = useCallback(
    async (data: FlowAuthorCreate | FlowAuthorUpdate) => {
      try {
        if (mode === "create") {
          await createMutation.mutateAsync(data as FlowAuthorCreate);
        } else if (mode === "update") {
          await updateMutation.mutateAsync(data as FlowAuthorUpdate);
        }
      } catch (error) {
  ...
  const handleAutosave = useCallback(
    async (data: Partial<FlowAuthorUpdate>) => {
      if (mode === "update" && id) {
        try {
          await updateMutation.mutateAsync(data as FlowAuthorUpdate);
        } catch (error) {
 ...
 */
export function FlowAuthorFormSubmit({
  children = "Submit",
  className,
  ...props
 ...
 */
export function FlowAuthorFormReset({
  children = "Reset",
  className,
  ...props
}: React.ButtonHTMLAttributes<HTMLButtonElement>) {
 ...
 */
export function FlowAuthorFormState({ className }: { className?: string }) {
