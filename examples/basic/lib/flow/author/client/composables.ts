...
export function useAuthorFormAutosave<
  T extends FlowAuthorCreate | FlowAuthorUpdate,
>(
  form: UseFormReturn<T>,
  mutation: UseMutationResult<FlowAuthor, Error, any>,
  options?: {
    enabled?: boolean;
    debounceMs?: number;
    fields?: string[];
  ...
  const saveField = useCallback(
    async (fieldName: string, value: any) => {
      console.log("[Author Autosave] saveField called:", {
        fieldName,
        value,
        enabled,
      ...
      if (lastSavedValues.current[fieldName] === value) {
        console.log("[Author Autosave] Value unchanged, skipping save");
        return;
      }
      ...
      console.log(
        "[Author Autosave] Setting debounce timer for",
        fieldName,
        "with delay",
        debounceMs,
      );
      debounceTimers.current[fieldName] = setTimeout(async () => {
        console.log("[Author Autosave] Debounce timer fired for", fieldName);
        ...
        if (lastSavedValues.current[fieldName] === currentValue) {
          console.log(
            "[Author Autosave] Value unchanged after debounce, skipping",
          );
          return;
        }
        ...
        if (!isValid) {
          console.log("[Author Autosave] Field validation failed");
        ...
        try {
          console.log("[Author Autosave] Calling mutation with:", {
          ...
          console.log("[Author Autosave] Mutation successful");
          ...
          console.error("[Author Autosave] Mutation failed:", error);
          setFieldSaveStates((prev) => ({ ...prev, [fieldName]: "error" }));
  ...
  useEffect(() => {
    console.log(
      "[Author Autosave] Setting up watch subscription, enabled:",
      enabled,
    );
    if (!enabled) return;

    const subscription = form.watch((value, { name, type }) => {
      console.log("[Author Autosave] Form change detected:", {
        name,
        type,
        isSystemUpdate: isSystemUpdate.current,
      });
      ...
      if (name && type === "change" && !isSystemUpdate.current) {
        ...
        console.log(
          "[Author Autosave] Triggering saveField for:",
          name,
          "with value:",
          fieldValue,
        );
    ...
    return () => {
      console.log("[Author Autosave] Cleaning up watch subscription");
...
export function useAuthorFormFieldTracking<
  T extends FlowAuthorCreate | FlowAuthorUpdate,
>(form: UseFormReturn<T>, original?: FlowAuthor) {
  const getFieldState = useCallback(
    (name: Path<T>) => {
      const fieldState = form.getFieldState(name);
      const currentValue = form.getValues(name);
      const originalValue = original?.[name as keyof FlowAuthor];
  ...
  const getDirtyFields = useCallback(() => {
    ...
    Object.keys(form.formState.dirtyFields).forEach((key) => {
      if ((form.formState.dirtyFields as any)[key]) {
        dirtyFields.push({
          field: key,
          original: original?.[key as keyof FlowAuthor],
...
export function useAuthorFormHistory<
  T extends FlowAuthorCreate | FlowAuthorUpdate,
>(
  form: UseFormReturn<T>,
  options?: {
...
export function useAuthorFieldDirect<
  T extends FlowAuthorCreate | FlowAuthorUpdate,
