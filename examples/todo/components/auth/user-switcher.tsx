...
import { ChevronDown, Users, Building, LogOut } from 'lucide-react';
import { useAuth, useUser, useTenant } from '@/lib/auth-context';
        ...
        <Button variant="ghost" className="flex items-center gap-2 px-2">
          <div className="flex items-center gap-2">
            ...
            <div className="hidden md:flex flex-col items-start">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs text-muted-foreground">{session.company.name}</span>
                <Badge variant="outline" className={`text-xs ${getPlanColor(session.company.plan)}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className="text-sm text-muted-foreground">{session.company.name}</span>
                <Badge variant="outline" className={`text-xs ${getPlanColor(session.company.plan)}`}>
                  {session.company.plan}
                </Badge>
              </div>
            ...
            <div className="flex items-center gap-2 w-full">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="outline" className={`text-xs ${getPlanColor(c.plan)}`}>
                    {c.plan}
                  </Badge>
                </div>
...
export function AuthDebugInfo() {
  ...
  return (
    <div className="fixed bottom-4 right-4 p-4 bg-black/90 text-white text-xs rounded-lg max-w-sm">
      ...
      <div>User ID: {flowCtx.user?.id}</div>
      <div>Roles: {flowCtx.user?.roles?.join(', ')}</div>
