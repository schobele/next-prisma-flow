...
export function useTodoListTemplateFormAutosave<
  T extends FlowTodoListTemplateCreate | FlowTodoListTemplateUpdate,
>(
  form: UseFormReturn<T>,
  mutation: UseMutationResult<FlowTodoListTemplate, Error, any>,
  options?: {
    enabled?: boolean;
    debounceMs?: number;
    fields?: string[];
  ...
  const saveField = useCallback(
    async (fieldName: string, value: any) => {
      console.log("[TodoListTemplate Autosave] saveField called:", {
        fieldName,
        value,
        enabled,
      ...
      if (lastSavedValues.current[fieldName] === value) {
        console.log(
          "[TodoListTemplate Autosave] Value unchanged, skipping save",
        );
        return;
      }
      ...
      console.log(
        "[TodoListTemplate Autosave] Setting debounce timer for",
        fieldName,
        "with delay",
        debounceMs,
      );
      debounceTimers.current[fieldName] = setTimeout(async () => {
        console.log(
          "[TodoListTemplate Autosave] Debounce timer fired for",
          fieldName,
        );
        ...
        if (lastSavedValues.current[fieldName] === currentValue) {
          console.log(
            "[TodoListTemplate Autosave] Value unchanged after debounce, skipping",
          );
          return;
        }
        ...
        if (!isValid) {
          console.log("[TodoListTemplate Autosave] Field validation failed");
        ...
        try {
          console.log("[TodoListTemplate Autosave] Calling mutation with:", {
          ...
          console.log("[TodoListTemplate Autosave] Mutation successful");
          ...
          console.error("[TodoListTemplate Autosave] Mutation failed:", error);
          setFieldSaveStates((prev) => ({ ...prev, [fieldName]: "error" }));
  ...
  useEffect(() => {
    console.log(
      "[TodoListTemplate Autosave] Setting up watch subscription, enabled:",
      enabled,
    );
    if (!enabled) return;

    const subscription = form.watch((value, { name, type }) => {
      console.log("[TodoListTemplate Autosave] Form change detected:", {
        name,
        type,
        isSystemUpdate: isSystemUpdate.current,
      });
      ...
      if (name && type === "change" && !isSystemUpdate.current) {
        ...
        console.log(
          "[TodoListTemplate Autosave] Triggering saveField for:",
          name,
          "with value:",
          fieldValue,
        );
    ...
    return () => {
      console.log("[TodoListTemplate Autosave] Cleaning up watch subscription");
...
export function useTodoListTemplateFormFieldTracking<
  T extends FlowTodoListTemplateCreate | FlowTodoListTemplateUpdate,
>(form: UseFormReturn<T>, original?: FlowTodoListTemplate) {
  const getFieldState = useCallback(
    (name: Path<T>) => {
      const fieldState = form.getFieldState(name);
      const currentValue = form.getValues(name);
      const originalValue = original?.[name as keyof FlowTodoListTemplate];
  ...
  const getDirtyFields = useCallback(() => {
    ...
    Object.keys(form.formState.dirtyFields).forEach((key) => {
      if ((form.formState.dirtyFields as any)[key]) {
        dirtyFields.push({
          field: key,
          original: original?.[key as keyof FlowTodoListTemplate],
...
export function useTodoListTemplateFormHistory<
  T extends FlowTodoListTemplateCreate | FlowTodoListTemplateUpdate,
>(
  form: UseFormReturn<T>,
  options?: {
...
export function useTodoListTemplateFieldDirect<
  T extends FlowTodoListTemplateCreate | FlowTodoListTemplateUpdate,
>(name: Path<T>, form: UseFormReturn<T>) {
