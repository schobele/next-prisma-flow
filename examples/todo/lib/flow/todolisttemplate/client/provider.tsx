...
/**
 * Props for the TodoListTemplate form component
 */
export interface FlowTodoListTemplateFormProps {
  children: ReactNode;
  ...
  defaultValues?: Partial<
    FlowTodoListTemplateCreate | FlowTodoListTemplateUpdate
  >;

  /**
   * Callback when form is successfully submitted
   */
  onSuccess?: (data: FlowTodoListTemplate) => void | Promise<void>;
...
/**
 * Type-safe form component for TodoListTemplate
 *
 * @example
 * ```tsx
 * <FlowTodoListTemplateForm mode="create">
 ...
 * </FlowTodoListTemplateForm>
 * ```
 */
export function FlowTodoListTemplateForm({
  children,
  mode,
  id,
  defaultValues,
  ...
  className,
  style,
}: FlowTodoListTemplateFormProps) {
  // Validate props
  if (mode === "update" && !id) {
    throw new Error(
      `FlowTodoListTemplateForm: id is required when mode is "update"`,
    );
  }

  // Fetch existing data for update mode
  const { data: existingData, isLoading: isLoadingData } = useTodoListTemplate(
    id || "",
    {
  ...
  const form = useForm<FlowTodoListTemplateCreate | FlowTodoListTemplateUpdate>(
    {
  ...
  const handleSubmit = useCallback(
    async (data: FlowTodoListTemplateCreate | FlowTodoListTemplateUpdate) => {
      try {
        if (mode === "create") {
          await createMutation.mutateAsync(data as FlowTodoListTemplateCreate);
        } else if (mode === "update") {
          await updateMutation.mutateAsync(data as FlowTodoListTemplateUpdate);
        }
      } catch (error) {
  ...
  const handleAutosave = useCallback(
    async (data: Partial<FlowTodoListTemplateUpdate>) => {
      if (mode === "update" && id) {
        try {
          await updateMutation.mutateAsync(data as FlowTodoListTemplateUpdate);
        } catch (error) {
 ...
 */
export function FlowTodoListTemplateFormSubmit({
  children = "Submit",
  className,
  ...props
 ...
 */
export function FlowTodoListTemplateFormReset({
  children = "Reset",
  className,
  ...props
}: React.ButtonHTMLAttributes<HTMLButtonElement>) {
 ...
 */
export function FlowTodoListTemplateFormState({
  className,
}: {
  className?: string;
}) {
