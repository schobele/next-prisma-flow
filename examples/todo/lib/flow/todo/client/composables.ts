...
export function useTodoFormAutosave<T extends FlowTodoCreate | FlowTodoUpdate>(
  form: UseFormReturn<T>,
  mutation: UseMutationResult<FlowTodo, Error, any>,
  options?: {
    enabled?: boolean;
    debounceMs?: number;
    fields?: string[];
  ...
  const saveField = useCallback(
    async (fieldName: string, value: any) => {
      console.log("[Todo Autosave] saveField called:", {
        fieldName,
        value,
        enabled,
      ...
      if (lastSavedValues.current[fieldName] === value) {
        console.log("[Todo Autosave] Value unchanged, skipping save");
        return;
      }
      ...
      console.log(
        "[Todo Autosave] Setting debounce timer for",
        fieldName,
        "with delay",
        debounceMs,
      );
      debounceTimers.current[fieldName] = setTimeout(async () => {
        console.log("[Todo Autosave] Debounce timer fired for", fieldName);
        ...
        if (lastSavedValues.current[fieldName] === currentValue) {
          console.log(
            "[Todo Autosave] Value unchanged after debounce, skipping",
          );
          return;
        }
        ...
        if (!isValid) {
          console.log("[Todo Autosave] Field validation failed");
        ...
        try {
          console.log("[Todo Autosave] Calling mutation with:", {
          ...
          console.log("[Todo Autosave] Mutation successful");
          ...
          console.error("[Todo Autosave] Mutation failed:", error);
          setFieldSaveStates((prev) => ({ ...prev, [fieldName]: "error" }));
  ...
  useEffect(() => {
    console.log(
      "[Todo Autosave] Setting up watch subscription, enabled:",
      enabled,
    );
    if (!enabled) return;

    const subscription = form.watch((value, { name, type }) => {
      console.log("[Todo Autosave] Form change detected:", {
        name,
        type,
        isSystemUpdate: isSystemUpdate.current,
      });
      ...
      if (name && type === "change" && !isSystemUpdate.current) {
        ...
        console.log(
          "[Todo Autosave] Triggering saveField for:",
          name,
          "with value:",
          fieldValue,
        );
    ...
    return () => {
      console.log("[Todo Autosave] Cleaning up watch subscription");
...
export function useTodoFormFieldTracking<
  T extends FlowTodoCreate | FlowTodoUpdate,
>(form: UseFormReturn<T>, original?: FlowTodo) {
  const getFieldState = useCallback(
    (name: Path<T>) => {
      const fieldState = form.getFieldState(name);
      const currentValue = form.getValues(name);
      const originalValue = original?.[name as keyof FlowTodo];
  ...
  const getDirtyFields = useCallback(() => {
    ...
    Object.keys(form.formState.dirtyFields).forEach((key) => {
      if ((form.formState.dirtyFields as any)[key]) {
        dirtyFields.push({
          field: key,
          original: original?.[key as keyof FlowTodo],
...
export function useTodoFormHistory<T extends FlowTodoCreate | FlowTodoUpdate>(
  form: UseFormReturn<T>,
  options?: {
...
export function useTodoFieldDirect<T extends FlowTodoCreate | FlowTodoUpdate>(
  name: Path<T>,
  form: UseFormReturn<T>,
) {
