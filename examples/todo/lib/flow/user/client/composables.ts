...
export function useUserFormAutosave<T extends FlowUserCreate | FlowUserUpdate>(
  form: UseFormReturn<T>,
  mutation: UseMutationResult<FlowUser, Error, any>,
  options?: {
    enabled?: boolean;
    debounceMs?: number;
    fields?: string[];
  ...
  const saveField = useCallback(
    async (fieldName: string, value: any) => {
      console.log("[User Autosave] saveField called:", {
        fieldName,
        value,
        enabled,
      ...
      if (lastSavedValues.current[fieldName] === value) {
        console.log("[User Autosave] Value unchanged, skipping save");
        return;
      }
      ...
      console.log(
        "[User Autosave] Setting debounce timer for",
        fieldName,
        "with delay",
        debounceMs,
      );
      debounceTimers.current[fieldName] = setTimeout(async () => {
        console.log("[User Autosave] Debounce timer fired for", fieldName);
        ...
        if (lastSavedValues.current[fieldName] === currentValue) {
          console.log(
            "[User Autosave] Value unchanged after debounce, skipping",
          );
          return;
        }
        ...
        if (!isValid) {
          console.log("[User Autosave] Field validation failed");
        ...
        try {
          console.log("[User Autosave] Calling mutation with:", {
          ...
          console.log("[User Autosave] Mutation successful");
          ...
          console.error("[User Autosave] Mutation failed:", error);
          setFieldSaveStates((prev) => ({ ...prev, [fieldName]: "error" }));
  ...
  useEffect(() => {
    console.log(
      "[User Autosave] Setting up watch subscription, enabled:",
      enabled,
    );
    if (!enabled) return;

    const subscription = form.watch((value, { name, type }) => {
      console.log("[User Autosave] Form change detected:", {
        name,
        type,
        isSystemUpdate: isSystemUpdate.current,
      });
      ...
      if (name && type === "change" && !isSystemUpdate.current) {
        ...
        console.log(
          "[User Autosave] Triggering saveField for:",
          name,
          "with value:",
          fieldValue,
        );
    ...
    return () => {
      console.log("[User Autosave] Cleaning up watch subscription");
...
export function useUserFormFieldTracking<
  T extends FlowUserCreate | FlowUserUpdate,
>(form: UseFormReturn<T>, original?: FlowUser) {
  const getFieldState = useCallback(
    (name: Path<T>) => {
      const fieldState = form.getFieldState(name);
      const currentValue = form.getValues(name);
      const originalValue = original?.[name as keyof FlowUser];
  ...
  const getDirtyFields = useCallback(() => {
    ...
    Object.keys(form.formState.dirtyFields).forEach((key) => {
      if ((form.formState.dirtyFields as any)[key]) {
        dirtyFields.push({
          field: key,
          original: original?.[key as keyof FlowUser],
...
export function useUserFormHistory<T extends FlowUserCreate | FlowUserUpdate>(
  form: UseFormReturn<T>,
  options?: {
...
export function useUserFieldDirect<T extends FlowUserCreate | FlowUserUpdate>(
  name: Path<T>,
  form: UseFormReturn<T>,
) {
