...
/**
 * Props for the User form component
 */
export interface FlowUserFormProps {
  children: ReactNode;
   ...
   */
  defaultValues?: Partial<FlowUserCreate | FlowUserUpdate>;

  /**
   * Callback when form is successfully submitted
   */
  onSuccess?: (data: FlowUser) => void | Promise<void>;
...
/**
 * Type-safe form component for User
 *
 * @example
 * ```tsx
 * <FlowUserForm mode="create">
 *   <UserFormField
 *     name="title"
 *     render={({ field }) => (
 *       <FormItem>
...
export function FlowUserForm({
  ...
  // Validate props
  if (mode === "update" && !id) {
    throw new Error(`FlowUserForm: id is required when mode is "update"`);
  }

  // Fetch existing data for update mode
  const { data: existingData, isLoading: isLoadingData } = useUser(id || "", {
  ...
  const handleSubmit = useCallback(
    async (data: FlowUserCreate | FlowUserUpdate) => {
      try {
        if (mode === "create") {
          await createMutation.mutateAsync(data as FlowUserCreate);
        } else if (mode === "update") {
          await updateMutation.mutateAsync(data as FlowUserUpdate);
        }
      } catch (error) {
  ...
  const handleAutosave = useCallback(
    async (data: Partial<FlowUserUpdate>) => {
      if (mode === "update" && id) {
        try {
          await updateMutation.mutateAsync(data as FlowUserUpdate);
        } catch (error) {
 ...
 */
export function FlowUserFormSubmit({
  children = "Submit",
  className,
  ...props
 ...
 */
export function FlowUserFormReset({
  children = "Reset",
  className,
  ...props
}: React.ButtonHTMLAttributes<HTMLButtonElement>) {
 ...
 */
export function FlowUserFormState({ className }: { className?: string }) {
