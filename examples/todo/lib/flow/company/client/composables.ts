...
export function useCompanyFormAutosave<
  T extends FlowCompanyCreate | FlowCompanyUpdate,
>(
  form: UseFormReturn<T>,
  mutation: UseMutationResult<FlowCompany, Error, any>,
  options?: {
    enabled?: boolean;
    debounceMs?: number;
    fields?: string[];
  ...
  const saveField = useCallback(
    async (fieldName: string, value: any) => {
      console.log("[Company Autosave] saveField called:", {
        fieldName,
        value,
        enabled,
      ...
      if (lastSavedValues.current[fieldName] === value) {
        console.log("[Company Autosave] Value unchanged, skipping save");
        return;
      }
      ...
      console.log(
        "[Company Autosave] Setting debounce timer for",
        fieldName,
        "with delay",
        debounceMs,
      );
      debounceTimers.current[fieldName] = setTimeout(async () => {
        console.log("[Company Autosave] Debounce timer fired for", fieldName);
        ...
        if (lastSavedValues.current[fieldName] === currentValue) {
          console.log(
            "[Company Autosave] Value unchanged after debounce, skipping",
          );
          return;
        }
        ...
        if (!isValid) {
          console.log("[Company Autosave] Field validation failed");
        ...
        try {
          console.log("[Company Autosave] Calling mutation with:", {
          ...
          console.log("[Company Autosave] Mutation successful");
          ...
          console.error("[Company Autosave] Mutation failed:", error);
          setFieldSaveStates((prev) => ({ ...prev, [fieldName]: "error" }));
  ...
  useEffect(() => {
    console.log(
      "[Company Autosave] Setting up watch subscription, enabled:",
      enabled,
    );
    if (!enabled) return;

    const subscription = form.watch((value, { name, type }) => {
      console.log("[Company Autosave] Form change detected:", {
        name,
        type,
        isSystemUpdate: isSystemUpdate.current,
      });
      ...
      if (name && type === "change" && !isSystemUpdate.current) {
        ...
        console.log(
          "[Company Autosave] Triggering saveField for:",
          name,
          "with value:",
          fieldValue,
        );
    ...
    return () => {
      console.log("[Company Autosave] Cleaning up watch subscription");
...
export function useCompanyFormFieldTracking<
  T extends FlowCompanyCreate | FlowCompanyUpdate,
>(form: UseFormReturn<T>, original?: FlowCompany) {
  const getFieldState = useCallback(
    (name: Path<T>) => {
      const fieldState = form.getFieldState(name);
      const currentValue = form.getValues(name);
      const originalValue = original?.[name as keyof FlowCompany];
  ...
  const getDirtyFields = useCallback(() => {
    ...
    Object.keys(form.formState.dirtyFields).forEach((key) => {
      if ((form.formState.dirtyFields as any)[key]) {
        dirtyFields.push({
          field: key,
          original: original?.[key as keyof FlowCompany],
...
export function useCompanyFormHistory<
  T extends FlowCompanyCreate | FlowCompanyUpdate,
>(
  form: UseFormReturn<T>,
  options?: {
...
export function useCompanyFieldDirect<
  T extends FlowCompanyCreate | FlowCompanyUpdate,
