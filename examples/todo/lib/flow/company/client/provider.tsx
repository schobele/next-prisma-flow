...
/**
 * Props for the Company form component
 */
export interface FlowCompanyFormProps {
  children: ReactNode;
   ...
   */
  defaultValues?: Partial<FlowCompanyCreate | FlowCompanyUpdate>;

  /**
   * Callback when form is successfully submitted
   */
  onSuccess?: (data: FlowCompany) => void | Promise<void>;
...
/**
 * Type-safe form component for Company
 *
 * @example
 * ```tsx
 * <FlowCompanyForm mode="create">
 *   <CompanyFormField
 *     name="title"
 *     render={({ field }) => (
 *       <FormItem>
 ...
 * ```
 */
export function FlowCompanyForm({
  children,
  mode,
  id,
  defaultValues,
  ...
  className,
  style,
}: FlowCompanyFormProps) {
  // Validate props
  if (mode === "update" && !id) {
    throw new Error(`FlowCompanyForm: id is required when mode is "update"`);
  }

  // Fetch existing data for update mode
  const { data: existingData, isLoading: isLoadingData } = useCompany(
    id || "",
    {
  ...
  const handleSubmit = useCallback(
    async (data: FlowCompanyCreate | FlowCompanyUpdate) => {
      try {
        if (mode === "create") {
          await createMutation.mutateAsync(data as FlowCompanyCreate);
        } else if (mode === "update") {
          await updateMutation.mutateAsync(data as FlowCompanyUpdate);
        }
      } catch (error) {
  ...
  const handleAutosave = useCallback(
    async (data: Partial<FlowCompanyUpdate>) => {
      if (mode === "update" && id) {
        try {
          await updateMutation.mutateAsync(data as FlowCompanyUpdate);
        } catch (error) {
 ...
 */
export function FlowCompanyFormSubmit({
  children = "Submit",
  className,
  ...props
 ...
 */
export function FlowCompanyFormReset({
  children = "Reset",
  className,
  ...props
}: React.ButtonHTMLAttributes<HTMLButtonElement>) {
 ...
 */
export function FlowCompanyFormState({ className }: { className?: string }) {
