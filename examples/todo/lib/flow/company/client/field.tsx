...
/**
 * Valid field names for Company forms
 */
export type CompanyFieldName =
  | Path<FlowCompanyCreate>
  | Path<FlowCompanyUpdate>;

/**
 * Props for CompanyFormField component
 */
export interface CompanyFormFieldProps extends Omit<FlowFieldProps, "name"> {
  /**
...
/**
 * Type-safe form field for Company
 ...
 * ```
 */
export function CompanyFormField({ name, ...props }: CompanyFormFieldProps) {
  return <FlowField name={name as any} {...props} />;
}

/**
 * Common field transforms for Company
 *
 * @example
 * ```tsx
 * <CompanyFormField
 *   name="price"
 *   transform={CompanyFieldTransforms.number}
 *   render={({ field }) => <Input type="number" {...field} />}
 * />
 * ```
 */
export const CompanyFieldTransforms = {
  /**
 ...
 *   const [title, setTitle] = useCompanyFieldValue("title");
 *   return <input value={title} onChange={e => setTitle(e.target.value)} />;
 * }
 * ```
 */
export function useCompanyFieldValue(name: CompanyFieldName) {
 ...
 * ```tsx
 * function MyField() {
 *   const fieldState = useCompanyFieldState("title");
 *   return (
 *     <div>
 ...
 * ```
 */
export function useCompanyFieldState(name: CompanyFieldName) {
  const { form } = useFlowFormContext();
 ...
 * ```tsx
 * function MyField() {
 *   const error = useCompanyFieldError("email");
 *   if (error) {
 ...
 * ```
 */
export function useCompanyFieldError(name: CompanyFieldName) {
  const { form } = useFlowFormContext();
