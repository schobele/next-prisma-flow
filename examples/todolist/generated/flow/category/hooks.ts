...
export function useCategories(autoFetch = true): UseCategoriesResult {
	...
	// Batch operations
	const createMany = useCallback(async (inputs: CategoryCreateInput[]) => {
	...
	return {
		...
		count,
		isEmpty,
		createCategory: useCallback(async (data: CategoryCreateInput) => await createCategory(data), [createCategory]),
		updateCategory: useCallback(
			async (id: string, data: CategoryUpdateInput) => await updateCategory({ id, data }),
			[updateCategory],
		),
		deleteCategory: useCallback(async (id: string) => await deleteCategory(id), [deleteCategory]),
		createMany,
		deleteMany,
...
export function useCategory(id: string): UseCategoryResult {
	...
	return {
		...
		error,
		update: useCallback(
			async (updateData: CategoryUpdateInput) => await updateCategory({ id, data: updateData }),
			[updateCategory, id],
		),
		delete: useCallback(async () => await deleteCategory(id), [deleteCategory, id]),
...
export function useCreateCategoryForm(initialData?: Partial<CategoryCreateInput>): UseCreateCategoryFormResult {
	...
	const validateField = useCallback(
		(fieldName: keyof CategoryCreateInput): boolean => {
			try {
	...
	const field = useCallback(
		(name: keyof CategoryCreateInput): CategoryFieldConfig => {
...
export function useUpdateCategoryForm(
	id: string,
	initialData?: Partial<CategoryUpdateInput>,
): UseUpdateCategoryFormResult {
	...
	// Validate individual field
	const validateField = useCallback(
		(fieldName: keyof CategoryUpdateInput): boolean => {
			try {
				const value = data[fieldName];
				if (value !== undefined && value !== null && value !== "") {
					setErrors((prev) => {
	...
	// Field helper function with internal memoization
	const field = useCallback(
		(name: keyof CategoryUpdateInput): CategoryFieldConfig => {
			const cacheKey = `${name}-${data[name]}-${errors[name as string]}-${touched[name as string]}`;
