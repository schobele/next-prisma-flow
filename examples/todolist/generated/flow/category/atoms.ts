...
export const refreshCategoriesAtom = atom(
  null,
  async (_get, set) => {
...
export const categoryByIdAtom = (id: string) => atom((get) => {
  const categoriesMap = get(baseCategoriesAtom);
...
export const optimisticCreateCategoryAtom = atom(
  ...
  async (get, set, categoryData: Parameters<typeof CategoryActions.createCategory>[0]) => {
    ...
    set(baseCategoriesAtom, (draft) => {
      draft[tempId] = optimisticCategory as Category;
    });
...
export const optimisticDeleteCategoryAtom = atom(
  null,
