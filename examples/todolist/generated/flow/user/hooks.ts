...
// Generated at: 2025-06-05T20:50:49.447Z
...
export function useUsers(autoFetch = true): UseUsersResult {
  ...
  return {
    ...
    createUser: useCallback(
      async (data: UserCreateInput) => await createUser(data),
      [createUser]
    ),
    updateUser: useCallback(
      async (id: string, data: UserUpdateInput) => 
        await updateUser({ id, data }),
      [updateUser]
    ),
    deleteUser: useCallback(
      async (id: string) => await deleteUser(id),
      [deleteUser]
    ),
    createMany,
    deleteMany,
...
export function useUser(id: string): UseUserResult {
  ...
  return {
    ...
    error,
    update: useCallback(
      async (updateData: UserUpdateInput) => 
...
export function useCreateUserForm(initialData?: Partial<UserCreateInput>): UseCreateUserFormResult {
  ...
  const validateField = useCallback((fieldName: keyof UserCreateInput): boolean => {
    try {
  ...
  const field = useCallback((name: keyof UserCreateInput): UserFieldConfig => {
...
export function useUpdateUserForm(id: string, initialData?: Partial<UserUpdateInput>): UseUpdateUserFormResult {
  ...
  // Validate individual field
  const validateField = useCallback((fieldName: keyof UserUpdateInput): boolean => {
    try {
      const value = data[fieldName];
      if (value !== undefined && value !== null && value !== '') {
        setErrors(prev => {
  ...
  // Field helper function with internal memoization
  const field = useCallback((name: keyof UserUpdateInput): UserFieldConfig => {
    const cacheKey = `${name}-${data[name]}-${errors[name as string]}-${touched[name as string]}`;
