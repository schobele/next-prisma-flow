...
import type { Todo, TodoCreateInput, TodoUpdateInput, TodoFormData, TodoFieldConfig } from "./types";
...
export function useTodos(autoFetch = true): UseTodosResult {
	...
	// Batch operations
	const createMany = useCallback(async (inputs: TodoCreateInput[]) => {
	...
	return {
		...
		count,
		isEmpty,
		createTodo: useCallback(async (data: TodoCreateInput) => await createTodo(data), [createTodo]),
		updateTodo: useCallback(async (id: string, data: TodoUpdateInput) => await updateTodo({ id, data }), [updateTodo]),
		deleteTodo: useCallback(async (id: string) => await deleteTodo(id), [deleteTodo]),
		createMany,
		deleteMany,
...
export function useTodo(id: string): UseTodoResult {
	...
	return {
		...
		error,
		update: useCallback(
			async (updateData: TodoUpdateInput) => await updateTodo({ id, data: updateData }),
			[updateTodo, id],
		),
		delete: useCallback(async () => await deleteTodo(id), [deleteTodo, id]),
